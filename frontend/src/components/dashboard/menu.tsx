        ...
        <ul className="flex flex-col min-h-[calc(100vh-48px-36px-16px-32px)] lg:min-h-[calc(100vh-32px-40px-32px)] items-start space-y-1 px-2">
          {menuList.map(({ groupLabel, menus }, index) => (
            <li className={cn("w-full", groupLabel ? "pt-5" : "")} key={index}>
              ...
              {menus.map(
                ({ href, label, icon: Icon, active, submenus,isScrollDown }, index) =>
                 href == "" && !submenus 
              ? 
                <div className="w-full" key={index}>
                  <CollapseMenuButton
                              ...
                              <Link href={href}>
                                ...
                                <p
                                  className={cn(
                                    ...
                                    onClick={(e) => {
                                      // Nếu đang ở cùng route, force reload
                                      if (pathname === href) {
                                        console.log("pahtname", pathname, "href", href);
                                        // e.preventDefault();
                                        window.location.href = href; // Hard reload
                                        // Hoặc dùng router.refresh() nếu chỉ muốn refresh data
                                        // router.refresh();
                                      }
                                    }}
                                >
