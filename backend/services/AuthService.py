...
class AuthService:
    ...
    async def get_history_detail(user_id: str, chat_id: str):
        # Lấy tài liệu chat từ cơ sở dữ liệu
        doc = await db["chat_history"].find_one({
            "_id": ObjectId(chat_id),
            "user_id": ObjectId(user_id)
        })

        # Nếu không tìm thấy lịch sử chat, trả về None
        if not doc:
            return None   

        # Lấy danh sách tất cả file trong thư mục store_image
        list_imgs = os.listdir("static")
        
        # Kiểm tra và thêm key "image" vào các message nếu có file_name tương ứng
        updated_messages = []
        for message in doc.get("messages", []):
            # Kiểm tra nếu message có trường "file_name"
            if "file_name" in message and message["file_name"] in list_imgs:
                # Tạo đường dẫn URL đến file ảnh
                image_url = f"http://localhost:8000/static/{message['file_name']}"
                # Cập nhật message thêm key "image" với giá trị là URL
                message["image"] = image_url

            # Thêm message đã cập nhật vào danh sách messages
